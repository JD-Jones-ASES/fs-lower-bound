...
arithmetic anywhere. A build-time audit runs \texttt{Lean.collectAxioms}
over the $126$ declarations of its audit list --- a set whose transitive
closure contains every public declaration of the development --- and fails
the build if any of them depends on an axiom outside Lean's three standard
axioms (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}). A source snapshot of the
